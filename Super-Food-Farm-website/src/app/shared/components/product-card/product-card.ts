...
export class ProductCard implements AfterViewInit, OnInit {
  ...
  applyFiltersAndSort() {
    ...
    let result = this.products.filter(product => {
      ...
      const priceMatch = product.price <= this.activeFilters.actual_price;
      const ratingMatch = this.activeFilters.star.length === 0 || this.activeFilters.star.some(r => product.rating >= r);
      return categoryMatch && priceMatch && ratingMatch;
    });

    // Sorting
    if (this.currentSort === 'price-low') result.sort((a, b) => a.actual_price - b.actual_price);
    else if (this.currentSort === 'price-high') result.sort((a, b) => b.actual_price - a.actual_price);
    else if (this.currentSort === 'rating') result.sort((a, b) => b.star - a.star);
    else if (this.currentSort === 'name-a') result.sort((a, b) => a.tag.localeCompare(b.tag));

    this.filteredProducts = result;
  }

  resetFilters() {
    this.activeFilters = { category: [], actual_price: 50, star: [], tag: [], product_sold: ['false'] };
    this.applyFiltersAndSort();
  }
